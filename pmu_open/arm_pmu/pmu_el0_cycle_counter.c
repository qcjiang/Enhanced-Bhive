...
static void
enable_cycle_event(void* data)
{
	u64 val = 0, event = *((u64*)(data)), r = 0;
//	printk(KERN_INFO "== data is  0x%llx\n",event);
//	u64 val = 0, event = 0x0011, r = 0;
//	event = 0x0011;
	...
	/* clear cycle counter and start */
//	asm volatile("mrs %0, pmcr_el0" : "=r" (val));
	asm volatile("msr pmcr_el0, %0" : : "r" (val | BIT(0) | BIT(1) | BIT(2)));

//	asm volatile("msr pmcr_el0, %0" : : "r" (0x0002));
...
{
	asm volatile("msr pmcr_el0, %0" : : "r" BIT(2));
...
//	on_each_cpu(enable_cycle_counter_el0, NULL, 1);
	...
	on_each_cpu(enable_cycle_event, &event, 1);
